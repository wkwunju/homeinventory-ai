...
export default function IconSelector({ selectedIcon, onIconSelect, onClose }: IconSelectorProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-xl w-full mx-2 sm:mx-4 max-w-4xl max-h-[88vh] overflow-hidden">
        {/* 头部 */}
              ...
              {filteredIcons.map((icon) => (
                ...
                >
                  <div className="text-center">
                    <div className="text-2xl sm:text-3xl mb-1 sm:mb-2">{icon.icon}</div>
                    <div className="text-xs sm:text-sm font-medium text-gray-900 truncate">{icon.name}</div>
                    <div className="text-[10px] sm:text-xs text-gray-500 truncate">{icon.description}</div>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="p-4 sm:p-6 border-t border-gray-200 bg-gray-50">
          <div className="flex justify-between items-center">
            ...
            <button
              onClick={onClose}
              className="px-3 sm:px-4 py-2 text-gray-600 hover:text-gray-800 hover:bg-gray-200 rounded-lg transition-colors"
            >
              取消
            </button>
          </div>
